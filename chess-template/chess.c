...
int getBoard(chessgame record, int moves);
int getFirstMoves(chessgame r, int moves);
int hasOpening(chessgame record, chessgame opening);    
//int hasBoard(chessgame record, chessgame board, int moves);
...
void getBoardStateFromGame(chessgame *game, int move_index, chessboard *board) {
    // Mock logic to simulate extracting board state based on move index
    snprintf(board->fen, sizeof(board->fen), "FEN at move %d", move_index);
}

// Function to convert the chessboard type to a string representation
void convertChessboardToString(chessboard *board, char *board_str) {
    // Mock logic to convert the chessboard to a string (FEN notation in this example)
    strncpy(board_str, board->fen, sizeof(board->fen));
}
